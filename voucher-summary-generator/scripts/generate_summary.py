...
用法：
    python generate_summary.py                           # 使用默认配置
    python generate_summary.py -i 12月总台.XLS.xlsx -t 600240  # 使用命令行参数
"""
...
def load_income_data(input_file, sheet_name):
    ...
    # 收入类型提取：名称首字母（Z/L/H/R/S/T），否则 None
    ...
    if '收入类型' in data.columns:
        ...
        print(f"收入类型自动校验: 共{len(data)}行，不一致 {mismatch_count} 行")
    else:
...
if __name__ == '__main__':
    # 支持命令行参数覆盖配置
    parser = argparse.ArgumentParser(description='总台财务数据汇总生成器')
    parser.add_argument('-i', '--input', default=INPUT_FILE, help='输入数据文件路径')
    parser.add_argument('-o', '--output', default=OUTPUT_FILE, help='输出文件路径')
    parser.add_argument('-t', '--transfer', type=float, default=TRANSFER_AMOUNT, help='转账金额')
    parser.add_argument('-s', '--sheet', default=SHEET_NAME, help='工作表名称')
    args = parser.parse_args()

    generate_summary(args.input, args.output, args.transfer, args.sheet)
